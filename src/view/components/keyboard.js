...
function renderLetterKey(letter) {
  return `<div class="skeuo-key w-8 h-8 sm:w-12 sm:h-12 text-sm sm:text-lg" data-key="${letter}"><span class="key-legend">${letter.toUpperCase()}</span></div>`;
}
...
export function renderKeyboard() {
  const rows = KEYBOARD_ROWS.map((row, rowIndex) => {
    ...
    return `<div class="flex justify-center gap-1 sm:gap-2 pr-4 sm:pr-8">${row.map(renderLetterKey).join('')}<div class="skeuo-key skeuo-key-wide px-3 sm:px-4 h-8 sm:h-12 text-xs sm:text-sm bg-red-900/20" data-key="Backspace"><span class="key-legend" data-i18n="backspace"></span></div></div>`;
  }).join('');
